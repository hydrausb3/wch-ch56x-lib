...
int main()
{
	...
	if (is_board1)
	{
		...
		serdes_init(SERDES_TYPE_DEVICE, sizeof(endp1_tx_buffer));
	}
	else
	{
		serdes_init(SERDES_TYPE_HOST, sizeof(endp1_tx_buffer));
		...
		while (1)
		{
			if (bsp_ubtn())
			{
				serdes_send(endp1_tx_buffer, sizeof(endp1_tx_buffer), 0);
			}
		}
	}
}
